...
<head>
    ...
    <title>{{ $odemeler->odeme_kodu_text }}-{{ $odemeler->odeme_kodu }}-{{ $odemeler->firmaadi->firma_unvan }} Ödeme Fişi</title>
...
==============================-->
                        <header class="th-header header-layout4">
                            <div class="row align-items-center justify-content-between">
                                ...
                                <div class="col-auto">
                                    <h1 class="big-title">Ödeme Fişi</h1>
                                    <p class="invoice-number"><b>Fiş No: </b>{{ $odemeler->odeme_kodu_text }}-{{ $odemeler->odeme_kodu }}</p>
                                    {{-- <p class="invoice-number"><b>satis No: </b>#935648</p> --}}
                                    <p class="invoice-number"><b>Fiş Tarihi: </b>{{ $odemeler->tarih }}</p>
                                </div>
                            </div>
                        </header>
                        {{-- <hr class="style1"> --}}
                        <div class="row justify-content-between" style="margin-top: -16px">
                            ...
                            <div class="col-auto">
                                <div class="invoice-right">
                                    <b>Müşteri:</b>
                                    <address>
                                        {{ $odemeler->firmaadi->firma_unvan }} <br>
                                        {{ $odemeler->firmaadi->yetkili_kisi }} <br>
                                        {{ $odemeler->firmaadi->yetkili_kisi_tel }} <br>
                                        {{ $odemeler->firmaadi->eposta }}
                                    </address>
                                </div>
                            </div>
                        </div>
                        <hr class="style1">
                        <p class="table-title text-center"><b>Ödeme Detayı:</b></p>
                        <table class="invoice-table table-stripe4 theme-color">
                            <thead>
                                <tr>
                                    <th style="width: 10px;  text-align: center;">#</th>
                                    <th style=" text-align: center;">Ödeme Yöntemi</th>
                                    <th style=" text-align: center;">Ödeme Türü</th>
                                    <th style=" text-align: center;">Ödeme Yapan</th>
                                    <th style=" text-align: center;"> Tahsil Eden</th>
                                    <th style=" text-align: center;">İlgili Kasa</th>
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td style=" text-align: center;">
                                        {{ $odemeler->odeme_tipi }}</td>
                                    <td style=" text-align: center;">
                                        {{ $odemeler->odeme_turu }}</td>
                                    <td style=" text-align: center;">{{ $odemeler->user->ad_soyad }}</td>
                                    <td style=" text-align: center;">{{ $odemeler->tahsil_eden }}</td>
                                    <td style=" text-align: center;">
                                        @if ($odemeler->odeme_tipi === 'Banka')
                                            {{ $odemeler->bankaadi->banka_adi }}
                                        @elseif($odemeler->odeme_tipi === 'Kasa')
                                            {{ $odemeler->kasaadi->kasa_adi }}
                                        @endif
                                    </td>
                                    <td style=" text-align: center;">{{number_format( $odemeler->odeme_tutar, 2, ',', '.') }} ₺</td>
                                </tr>
                            </tbody>
                        </table>
                            ...
                            <div class="col-md-12">
                                <table class="total-table">
                                    <tr>
                                        ...
                                        <td>{{ number_format($odemeler->satis_iskonto_toplam, 2, ',', '.') }} ₺</td>
                                    </tr>
                                    <tr>
                                        <th>TOPLAM KDV</th>
                                        <td>{{ number_format($odemeler->satis_kdv_toplam, 2, ',', '.') }} ₺</td>
                                    </tr>
                                    <tr>
                                        <th>ARA TOPLAM</th>
                                        <td>{{ number_format($odemeler->satis_ara_toplam, 2, ',', '.') }} ₺</td>
                                    </tr>
                                    <tr>
                                        <th>TOPLAM TUTAR</th>
                                        <td>{{ number_format($odemeler->satis_kdvli_toplam, 2, ',', '.') }} ₺</td>
                                    </tr>
                                </table>
                            </div>
                        </div> --}}
            ...
            */
            ...
            $('#download_btn').on('click', function() {
                ...
                html2canvas(downloadSection[0], {
                    ...
                    var pdfUrl =
                        '{{ $odemeler->odeme_kodu_text }}-{{ $odemeler->odeme_kodu }}-{{ $odemeler->firmaadi->firma_unvan }} Ödeme Fişi.pdf';
                    pdf.save(pdfUrl);
                });
            });
