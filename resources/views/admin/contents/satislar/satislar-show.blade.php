...
<head>
    ...
    <title>{{$satislar->satis_kodu_text}}-{{$satislar->satis_kodu}}-{{$satislar->firmaadi->firma_unvan}} Satış Faturası</title>
...
==============================-->
                            ...
                            <div class="row align-items-center justify-content-between">
                                <div class="col-auto">
                                    <div class="header-logo">
                                        <a href="index.html"><img src="{{ asset('logintemp/cukurovalogo.png') }}"
                                                style="width: auto; height: 72px;"></a>
                                    </div>
                                </div>
                                <div class="col-auto">
                                    <h1 class="big-title">Satış Faturası</h1>
                                    <p class="invoice-number"><b>Fatura No: </b>#935648</p>
                                    {{-- <p class="invoice-number"><b>satis No: </b>#935648</p> --}}
                                    <p class="invoice-number"><b>Fatura Tarihi: </b>{{$satislar->satis_tarihi}}</p>
                        ...
                        <div class="row justify-content-between" style="margin-top: -16px">
                            ...
                            <div class="col-auto">
                                <div class="invoice-right">
                                    <b>Müşteri:</b>
                                    <address>
                                        {{ $satislar->firmaadi->firma_unvan }} <br>
                                        {{ $satislar->firmaadi->yetkili_kisi }} <br>
                                        {{ $satislar->firmaadi->yetkili_kisi_tel }} <br>
                                        {{ $satislar->firmaadi->eposta }}
                                    </address>
                                </div>
                            </div>
                        </div>
                        ...
                        <table class="invoice-table table-stripe4 theme-color">
                            ...
                            @endphp
                            <tbody>
                                @foreach ($satislardata as $sn => $satislardataitem)
                                @php
                                        $birim_fiyat += $satislardataitem->satis_fiyat;
                                        $indirim_tutar += $satislardataitem->satis_iskonto;
                                        $sonuc = $birim_fiyat - $indirim_tutar;
                                    @endphp
                                    <tr>
                                        <td><strong>{{ $sn + 1 }}</strong></td>
                                        {{-- <td style=" text-align: center;">
                                            {{ $satislardataitem->hizmetlerkategori->kategori_ad }}</td> --}}
                                        <td style=" text-align: center;">
                                            {{ $satislardataitem->hizmetler->hizmet_ad }}</td>
                                            <td style=" text-align: center;">
                                                {{ $satislardataitem->satir_aciklama }}</td>
                                            <td style=" text-align: center;">
                                                {{ $satislardataitem->satis_hizmet_miktar }} {{ $satislardataitem->satis_hizmet_birim }}</td>
                                            <td style=" text-align: center;"> {{  number_format($satislardataitem->satis_fiyat, 2, ',', '.') }} ₺</td>
                                            <td style=" text-align: center;">
                                                {{ number_format($satislardataitem->satis_iskonto, 2, ',', '.') }} ₺</td>
                                                <td style=" text-align: center;">{{ number_format($sonuc, 2, ',', '.') }} ₺
                                                </td>
                                                <td style=" text-align: center;">
                                                    {{ number_format($satislardataitem->satis_kdv_tutar, 2, ',', '.') }} ₺
                                                    (%{{ $satislardataitem->satis_kdv_oran }})
                                                </td>
                                                <td style=" text-align: center;">
                                                    {{ number_format($satislardataitem->satis_toplam_fiyat, 2, ',', '.') }}
                                                    ₺</td>
                                    </tr>
                                @endforeach
                            </tbody>
                            ...
                            <div class="col-md-12">
                                <table class="total-table">
                                    <tr>
                                        ...
                                        <td>{{ number_format($satislar->satis_iskonto_toplam, 2, ',', '.') }} ₺</td>
                                    </tr>
                                    <tr>
                                        <th>TOPLAM KDV</th>
                                        <td>{{ number_format($satislar->satis_kdv_toplam, 2, ',', '.') }} ₺</td>
                                    </tr>
                                    <tr>
                                        <th>ARA TOPLAM</th>
                                        <td>{{ number_format($satislar->satis_ara_toplam, 2, ',', '.') }} ₺</td>
                                    </tr>
                                    <tr>
                                        <th>TOPLAM TUTAR</th>
                                        <td>{{ number_format($satislar->satis_kdvli_toplam, 2, ',', '.') }} ₺</td>
                                    </tr>
                                </table>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <p style="text-align: justify">Marka Başvurusu ile Markanın
                                    Yayına çıkması veya reddedilip edilmeyeceği süre yaklaşık olarak 3-4 aydır.
                                    Marka yayına çıkar ise 3.kişilerin itirazı için yine 2 ay yayında kaldıktan
                                    sonra başvurudan itibaren 4-8 ay sonra tescil kararı verilir ve gerekli harçlar
                                    ödendikten sonra da 1 ay içerisinde tescil belgesi tarafınıza teslim edilir.
                                    ...
                                    Çevrimiçi İşlemler sayfasından elde edilen araştırma sonuçları bilgi amaçlı
                                    olup, bu sonuçlara göre yapılacak başvurular "tescil edilir ya da edilemez"
                                    kesin yargısına varılmamalıdır.</p>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md-6" style="border: solid 1px black">
                                <div class="invoice-note2" style="text-align: center">
                                    <b>DANIŞMAN FİRMA <br></b>
                                    <div class="row">
                                        <div class="col-md-6">
                                            <p class="mb-0">OSMAN ÇEKİRGE<br></p>
                                            <div style="height: 150px; border-bottom: 1px solid black; margin: 10px 20px;"></div>
                                        </div>
                                        <div class="col-md-6">
                                            <p class="mb-0">MEHMET ASİ<br></p>
                                            <div style="height: 150px; border-bottom: 1px solid black; margin: 10px 20px;"></div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="col-md-6" style="border: solid 1px black">
                                <div class="invoice-note2" style="text-align: center">
                                    <b>MÜŞTERİ FİRMA <br></b>
                                    <div class="row">
                                        <div class="col-md-12">
                                            <p class="mb-0">İSİM SOYİSİM İMZA KAŞE<br></p>
                                            <div style="height: 150px; border-bottom: 1px solid black; margin: 10px 20px;"></div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="row pt-15" >
                            <div class="col-md-12">
                                <p style="text-align: center">Adres: Sümer Mahallesi Bülent Angın Bulvarı 69051 Sokak Hatice Hatun Apartmanı No:1 (Denizbank Üstü) 0322 225 8233 <br> Adres: Kavaslı Mahallesi Atatürk Bulvarı Ahmet Gürses Apt. No:81/9 Tel: 444 8 148 Antakya/Hatay/Türkiye
                                   <br> Adres: İstanbul Cad. No:5 Pelin İş Merkezi Kat:4 No:121 ( Nimet Abla Gişesi Üstü ) Tel: 444 8 148 Bakırköy - İstanbul Banka Hesap No: Adana Barajyolu İşbank Şubesi: TR030006400000160130862216 İş Bankası Esenler Şubesi:TR700006400000110980696588
                                   <br> Web: www.cukurovapatent.com / info@cukurovapatent.com / adana@cukurovapatent.com / istanbul@cukurovapatent.com
                                    </p>
                            </div>
                        </div>
                    ...
                    <div class="invoice-buttons">
                        ...
                        {{-- <button id="download_excel_btn" class="download_btn">
                            <svg width="25" height="19" viewBox="0 0 25 19" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <path
                                    d="M8.94531 11.1797C8.6849 10.8932 8.6849 10.6068 8.94531 10.3203C9.23177 10.0599 9.51823 10.0599 9.80469 10.3203L11.875 12.3516V6.375C11.901 5.98438 12.1094 5.77604 12.5 5.75C12.8906 5.77604 13.099 5.98438 13.125 6.375V12.3516L15.1953 10.3203C15.4818 10.0599 15.7682 10.0599 16.0547 10.3203C16.3151 10.6068 16.3151 10.8932 16.0547 11.1797L12.9297 14.3047C12.6432 14.5651 12.3568 14.5651 12.0703 14.3047L8.94531 11.1797ZM10.625 0.75C11.7969 0.75 12.8646 1.01042 13.8281 1.53125C14.8177 2.05208 15.625 2.76823 16.25 3.67969C16.8229 3.39323 17.4479 3.25 18.125 3.25C19.375 3.27604 20.4036 3.70573 21.2109 4.53906C22.0443 5.34635 22.474 6.375 22.5 7.625C22.5 8.01562 22.4479 8.41927 22.3438 8.83594C23.151 9.2526 23.7891 9.85156 24.2578 10.6328C24.7526 11.4141 25 12.2865 25 13.25C24.974 14.6562 24.4922 15.8411 23.5547 16.8047C22.5911 17.7422 21.4062 18.224 20 18.25H5.625C4.03646 18.1979 2.70833 17.651 1.64062 16.6094C0.598958 15.5417 0.0520833 14.2135 0 12.625C0.0260417 11.375 0.377604 10.2812 1.05469 9.34375C1.73177 8.40625 2.63021 7.72917 3.75 7.3125C3.88021 5.4375 4.58333 3.88802 5.85938 2.66406C7.13542 1.4401 8.72396 0.802083 10.625 0.75ZM10.625 2C9.08854 2.02604 7.78646 2.54688 6.71875 3.5625C5.67708 4.57812 5.10417 5.85417 5 7.39062C4.94792 7.91146 4.67448 8.27604 4.17969 8.48438C3.29427 8.79688 2.59115 9.33073 2.07031 10.0859C1.54948 10.8151 1.27604 11.6615 1.25 12.625C1.27604 13.875 1.70573 14.9036 2.53906 15.7109C3.34635 16.5443 4.375 16.974 5.625 17H20C21.0677 16.974 21.9531 16.6094 22.6562 15.9062C23.3594 15.2031 23.724 14.3177 23.75 13.25C23.75 12.5208 23.5677 11.8698 23.2031 11.2969C22.8385 10.724 22.3568 10.2682 21.7578 9.92969C21.2109 9.59115 21.0026 9.09635 21.1328 8.44531C21.2109 8.21094 21.25 7.9375 21.25 7.625C21.224 6.73958 20.9245 5.9974 20.3516 5.39844C19.7526 4.82552 19.0104 4.52604 18.125 4.5C17.6302 4.5 17.1875 4.60417 16.7969 4.8125C16.1719 5.04688 15.651 4.90365 15.2344 4.38281C14.7135 3.65365 14.0495 3.08073 13.2422 2.66406C12.4609 2.22135 11.5885 2 10.625 2Z"
...
{{-- <script>
  $('#download_excel_btn').on('click', function () {
      var table = document.getElementById('download_section'); // İndirilmek istenen tablo
      var wb = XLSX.utils.book_new();
      var ws = XLSX.utils.table_to_sheet(table);
      XLSX.utils.book_append_sheet(wb, ws, "Sayfa1");

      var fileName = '{{$satislar->satis_kodu_text}}-{{$satislar->satis_kodu}}-{{$satislar->firmaadi->firma_unvan}} Satış Faturası.xlsx';
    ...
    */
    ...
    $('#download_btn').on('click', function () {
      ...
      html2canvas(downloadSection[0], { allowTaint: true }).then(function (
        ...
        var pdfUrl = '{{$satislar->satis_kodu_text}}-{{$satislar->satis_kodu}}-{{$satislar->firmaadi->firma_unvan}} Satış Faturası.pdf';
        pdf.save(pdfUrl);
      });
    });
    ...
    $('#download_excel_btn').on('click', function () {
                var table = document.getElementById('download_section'); // Tabloyu al
                var wb = XLSX.utils.book_new(); // Yeni Excel dosyası oluştur
                var ws = XLSX.utils.table_to_sheet(table); // HTML Tablosunu Excel'e çevir
                XLSX.utils.book_append_sheet(wb, ws, "Satış Faturası"); // Sayfa adı ekle

                var fileName = '{{$satislar->satis_kodu_text}}-{{$satislar->satis_kodu}}-{{$satislar->firmaadi->firma_unvan}} Satış Faturası.xlsx';
                XLSX.writeFile(wb, fileName); // Dosyayı indir
            });
