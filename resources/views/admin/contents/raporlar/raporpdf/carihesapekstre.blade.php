...
==============================-->
                        <header class="th-header header-layout4">
                            <div class="row align-items-center justify-content-between">
                                ...
                                <div class="col-auto">
                                    <h1 class="big-title">Cari Hesap Ekstresi</h1>
                                    <p class="invoice-number"><b>Ekstre Tarihi: </b>{{$islem_tarihi}}</p>
                                    {{-- <p class="invoice-number"><b>satis No: </b>#935648</p> --}}
                                    <p class="invoice-number"><b>Ekstre No: </b></p>
                                </div>
                            </div>
                        </header>
                        {{-- <hr class="style1"> --}}
                            ...
                            <div class="col-auto">
                                <div class="invoice-right">
                                    ...
                                    @else
                                        <b>Müşteri:</b>
                                        <address>
                                            @if ($carihesapekstre->first() && $carihesapekstre->first()->firmaadi)
                                                {{ $carihesapekstre->first()->firmaadi->firma_unvan }} <br>
                                                {{ $carihesapekstre->first()->firmaadi->yetkili_kisi }} <br>
                                                {{ $carihesapekstre->first()->firmaadi->yetkili_kisi_tel }} <br>
                                                {{ $carihesapekstre->first()->firmaadi->eposta }}
                                            @else
                                                Firmaya ait kayıt bulunamadı.
                                            @endif
                                        </address>
                                    @endif
                                </div>
                    ...
                    @endphp
                        <table class="invoice-table table-stripe4 theme-color">
                            ...
                            <tbody>
                                @foreach ($carihesapekstre as $sn => $carihesapekstreitem)
                                    <tr>
                                        <td style=" text-align: center;"><strong>{{ $sn + 1 }}</strong></td>

                                        <td >
                                            @if($carihesapekstreitem->islem == 'Alış')
                                                <a href="{{ url('alislar/' . $carihesapekstreitem->alis_id) }}">{{ $carihesapekstreitem->islem }}</a>
                                            @elseif ($carihesapekstreitem->islem == 'Satış')
                                            <a href="{{ url('satislar/' . $carihesapekstreitem->satis_id) }}">{{ $carihesapekstreitem->islem }}</a>--({{$carihesapekstreitem->satis->satis_konu}})
                                            @elseif ($carihesapekstreitem->islem == 'Tahsilat')
                                            <a href="{{ url('tahsilat/' . $carihesapekstreitem->tahsilat_id) }}">{{ $carihesapekstreitem->islem }}</a>
                                            @elseif ($carihesapekstreitem->islem == 'Ödeme')
                                            <a href="{{ url('odemeler/' . $carihesapekstreitem->odeme_id) }}">{{ $carihesapekstreitem->islem }}</a>
                                            @elseif ($carihesapekstreitem->islem == 'Çek Tahsilatı')
                                            <a href="{{ url('tahsilat/' . $carihesapekstreitem->tahsilat_id) }}">{{ $carihesapekstreitem->islem }}</a>
                                            @elseif ($carihesapekstreitem->islem == 'Çek Ödemesi')
                                            <a href="{{ url('odemeler/' . $carihesapekstreitem->odeme_id) }}">{{ $carihesapekstreitem->islem }}</a>

                                            @else
                                            {{ $carihesapekstreitem->islem }}
                                            @endif
                                        </td>
                                            <td style=" text-align: center;">
                                                {{ $carihesapekstreitem->islem_tarihi }}</td>
                                                @if (!$cari_id)
                                                <td style=" text-align: center;">
                                                    {{ $carihesapekstreitem->firmaadi->firma_unvan }}
                                                </td>
                                                @else

                                                @endif
                                        <td style=" text-align: center;">{{ number_format($carihesapekstreitem->borc, 2, ',', '.') }} ₺</td>

                                        <td style=" text-align: center;">{{number_format( $carihesapekstreitem->alacak, 2, ',', '.') }} ₺
                                        </td>
                                    </tr>
                                    @php
                                        $borc_toplam += $carihesapekstreitem->borc;
                                        $alacak_toplam += $carihesapekstreitem->alacak;
                        ...
                        <div class="row" style="float: right">
                            <div class="col-md-12">
                                <table class="total-table">
                                    <tr>
                                        <th>TOPLAM BORÇ</th>
                                        <td>{{ number_format($borc_toplam, 2, ',', '.') }} ₺</td>
                                    </tr>
                                    <tr>
                                        <th>TOPLAM ALACAK</th>
                                        <td>{{ number_format($alacak_toplam, 2, ',', '.') }} ₺</td>
                                    </tr>
                                    <tr>
                                        <th>BAKİYE</th>
    ...
    */
    ...
    $('#download_btn').on('click', function () {
      ...
      html2canvas(downloadSection[0], { allowTaint: true }).then(function (
        ...
        @php
        $firmaUnvan = optional($carihesapekstre->first())->firmaadi->firma_unvan ?? 'FirmaBilgisiYok';
        @endphp

        var pdfUrl = '{{ $firmaUnvan }}-Hesap-Ekstresi.pdf';
        pdf.save(pdfUrl);
      });
    });
