...
==============================-->
                        <header class="th-header header-layout4">
                            <div class="row align-items-center justify-content-between">
                                ...
                                <div class="col-auto">
                                    <h1 class="big-title">Tahsilat Rapor</h1>
                            ...
                            <div class="col-auto">
                                <div class="invoice-right">
                                    ...
                                    <address>
                                        ...
                                        {{ $tahsilatrapor->first()->firmaadi->firma_unvan }}
                                        <br>
                                        {{ $tahsilatrapor->first()->firmaadi->yetkili_kisi }}
                                        <br>
                                        {{ $tahsilatrapor->first()->firmaadi->yetkili_kisi_tel }}
                                        <br>
                                        {{ $tahsilatrapor->first()->firmaadi->eposta }}
                                        @endif
                        ...
                        <table class="invoice-table table-stripe4 theme-color">
                            <thead>
                                <tr>
                                    ...
                                    <th>Ödeme Türü</th>
                                    <th>Ödeme Yapan</th>
                                    <th>Banka/Kasa İsmi</th>
                                    <th>Tahsilat Tutar</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach ($tahsilatrapor as $sn => $tahsilatraporitem)
                                    <tr>
                                        <td style=" text-align: center;"><strong>{{ $sn + 1 }}</strong></td>
                                        <td style=" text-align: center;">  <a href="{{ url('tahsilat/' . $tahsilatraporitem->tahsilat_id) }}">{{ $tahsilatraporitem->tahsilat->tahsilat_kodu_text }}-{{ $tahsilatraporitem->tahsilat->tahsilat_kodu }}</a></td>
                                        <td style=" text-align: center;">{{ $tahsilatraporitem->islem_tarihi }}</td>
                                        @if (!$cari_id)
                                        <td style=" text-align: center;">
                                            {{ $tahsilatraporitem->firmaadi->firma_unvan }}
                                        </td>
                                        @else

                                        @endif
                                        <td style=" text-align: center;">{{ $tahsilatraporitem->tahsilat->odeme_turu }}</td>
                                        <td style=" text-align: center;">{{ $tahsilatraporitem->tahsilat->odeme_yapan }}</td>
                                        <td style=" text-align: center;">
                                            @if ( $tahsilatraporitem->tahsilat->odeme_tipi == 'Banka')
                                                {{ $tahsilatraporitem->tahsilat->bankaadi->banka_adi }}
                                            @elseif( $tahsilatraporitem->tahsilat->odeme_tipi == 'Kasa')
                                                    {{ $tahsilatraporitem->tahsilat->kasaadi->kasa_adi}}
                                            @endif
                                        </td>


                                        <td style=" text-align: center;">{{ number_format($tahsilatraporitem->tahsilat->tahsilat_tutar, 2, ',', '.') }} ₺</td>
                                    </tr>
                                    @php
                                        $tahsilat_toplam += $tahsilatraporitem->tahsilat->tahsilat_tutar;
                                    @endphp
                                @endforeach
                            </tbody>
                        </table>
