...
==============================-->
                        <header class="th-header header-layout4">
                            <div class="row align-items-center justify-content-between">
                                ...
                                <div class="col-auto">
                                    <h1 class="big-title">Gider Rapor</h1>
                            ...
                            <div class="col-auto">
                                <div class="invoice-right">
                                    ...
                                    <address>
                                        ...
                                        {{ $giderrapor->first()->firmaadi->firma_unvan }}
                                        <br>
                                        {{ $giderrapor->first()->firmaadi->yetkili_kisi }}
                                        <br>
                                        {{ $giderrapor->first()->firmaadi->yetkili_kisi_tel }}
                                        <br>
                                        {{ $giderrapor->first()->firmaadi->eposta }}
                                        @endif
                        ...
                        <table class="invoice-table table-stripe4 theme-color">
                            <thead>
                                <tr>
                                    ...
                                    <th>Gider İsmi</th>
                                    <th>Miktar</th>
                                    <th>Gider Toplam Tutar</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach ($giderrapor as $sn => $giderraporitem)
                                    <tr>
                                        <td style="text-align: center;"><strong>{{ $sn + 1 }}</strong></td>
                                        <td style="text-align: center;">
                                            <a href="{{ url('alislar/' . $giderraporitem->alis_id) }}" target="_blank">
                                                {{ $giderraporitem->alis->alis_kodu_text }}-{{ $giderraporitem->alis->alis_kodu }}
                                            </a>
                                        </td>
                                        <td style="text-align: center;">{{ $giderraporitem->islem_tarihi }}</td>
                                        @if (!$cari_id)
                                        <td style=" text-align: center;">
                                            {{ $giderraporitem->firmaadi->firma_unvan }}
                                        </td>
                                        @else
                                        @endif
                                        {{-- Alislardata'daki Gider Adlarını Virgülle Ayırarak Tek Hücrede Göster --}}
                                        <td style="text-align: center;">
                                            {{ $giderraporitem->alis->alislardata->pluck('gider_adi')->join(', ') }}
                                        </td>

                                        {{-- Alislardata'daki Miktarları Virgülle Ayırarak Tek Hücrede Göster --}}
                                        <td style="text-align: center;">
                                            {{ $giderraporitem->alis->alislardata->pluck('miktar')->join(', ') }}
                                        </td>
                                        {{-- @foreach ($giderrapor as $sn => $giderraporitem)
                                    <tr>
                                        <td style=" text-align: center;"><strong>{{ $sn + 1 }}</strong></td>
                                        <td style=" text-align: center;">  <a href="{{ url('alislar/' . $giderraporitem->alis_id) }}">{{ $giderraporitem->alis->alis_kodu_text }}-{{ $giderraporitem->alis->alis_kodu }}</a></td>
                                        <td style=" text-align: center;">{{ $giderraporitem->islem_tarihi }}</td>
                                        @foreach ($giderraporitem->alis->alislardata as $alisitem )
                                        <td style=" text-align: center;">{{ $alisitem->gider->gider_adi }}</td>
                                        @endforeach
                                        @foreach ($giderraporitem->alis->alislardata as $alisitem )
                                        <td style=" text-align: center;">{{ $alisitem->miktar }}</td>
                                        @endforeach

                                @endforeach --}}
                                        <td style="text-align: center;">{{ number_format($giderraporitem->alis->toplam_tutar, 2, ',', '.') }} ₺</td>
                                    </tr>
                                    @php
                                        $odeme_toplam += $giderraporitem->alis->toplam_tutar;
                                    @endphp
                                @endforeach
                            </tbody>
                        </table>
                        ...
                        <p class="table-title text-center mt-35"><b>En Çok Alış Yapılan 5 Firma:</b></p>
                        <table class="invoice-table table-stripe4 theme-color">
                            <thead>
                                <tr>
                                    <th style="width: 15px;  text-align: center;">#</th>
                                    <th>Firma Adı</th>
                                    <th>Toplam Alış Tutar</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach ($enCokAlisFirmalari as $sn => $enCokAlisFirmalariitem)
                                    <tr>
                                        <td style=" text-align: center;"><strong>{{ $sn + 1 }}</strong></td>
                                        <td style=" text-align: center;"><a
                                                href="{{ url('cariler/' . $enCokAlisFirmalariitem->cari_id) }}" target="_blank">{{ $enCokAlisFirmalariitem->firma_unvan }}</a>
                                        </td>
                                        <td style=" text-align: center;">
                                            {{ number_format($enCokAlisFirmalariitem->toplam_alis, 2, ',', '.') }} ₺
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
