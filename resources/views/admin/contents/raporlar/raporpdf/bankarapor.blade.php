...
==============================-->
                        <header class="th-header header-layout4">
                            <div class="row align-items-center justify-content-between">
                                ...
                                <div class="col-auto">
                                    <h1 class="big-title">Banka Rapor</h1>
                                    <p class="invoice-number"><b>Rapor Tarihi: </b>{{$islem_tarihi}}</p>
                                    <p class="invoice-number"><b>Rapor No: </b>#935648</p>
                                    {{-- <p class="invoice-number"><b>Fatura Tarihi: </b></p> --}}
                                </div>
                            </div>
                        </header>
                        {{-- <hr class="style1"> --}}
                        <div class="row justify-content-between" style="margin-top: -16px">
                            ...
                            <div class="col-auto">
                                <div class="invoice-right">
                                    ...
                                    <b>BANKA ADI:</b>
                                    <address>
                                        {{ $bankarapor->first()->bankahrkt->banka->banka_adi }}
                                        <br>
                                    @endif
                                </address>
                                </div>
                        ...
                        @php
                        // Banka bazında son güncel bakiyeyi tutacak dizi
                        $bankalarSonGuncelBakiye = [];
                        @endphp
                        <table class="invoice-table table-stripe4 theme-color">
                            ...
                            <tbody>
                                @foreach ($bankarapor as $sn => $bankaraporitem)
                                    <tr>
                                        <td style=" text-align: center;"><strong>{{ $sn + 1 }}</strong></td>
                                        <td style=" text-align: center;">{{ $bankaraporitem->islem_tarihi }}</td>
                                        @if (!$bankalar)
                                            <td style=" text-align: center;">
                                                {{ $bankaraporitem->bankahrkt->banka->banka_adi}}
                                            </td>
                                        @endif
                                        <td style=" text-align: center;">
                                            @if (isset($bankaraporitem->bankahrkt->odeme_id))
                                                {{ $bankaraporitem->bankahrkt->odeme->user->ad_soyad}}
                                            @elseif (isset($bankaraporitem->bankahrkt->tahsilat_id))
                                            {{ $bankaraporitem->bankahrkt->tahsilat->odeme_yapan}}
                                            @endif
                                        </td>
                                        <td style=" text-align: center;">{{ number_format($bankaraporitem->bankahrkt->bakiye, 2, ',', '.') }} ₺</td>
                                        <td style=" text-align: center;">
                                            @if ($bankaraporitem->bankahrkt->hareket_tipi == 'Giden Havale')
                                              <span style="color: red">{{ number_format($bankaraporitem->bankahrkt->eklenen_tutar, 2, ',', '.') }} ₺</span>
                                              @elseif ($bankaraporitem->bankahrkt->hareket_tipi == 'Giden EFT')
                                              <span style="color: red">{{ number_format($bankaraporitem->bankahrkt->eklenen_tutar, 2, ',', '.') }} ₺</span>
                                            @elseif ($bankaraporitem->bankahrkt->hareket_tipi == 'Gelen EFT')
                                            <span style="color: green">{{ number_format($bankaraporitem->bankahrkt->eklenen_tutar, 2, ',', '.') }} ₺</span>
                                            @elseif ($bankaraporitem->bankahrkt->hareket_tipi == 'Gelen Havale')
                                            <span style="color: green">{{ number_format($bankaraporitem->bankahrkt->eklenen_tutar, 2, ',', '.') }} ₺</span>
                                            @endif</td>

                                        <td style=" text-align: center;">{{ number_format($bankaraporitem->bankahrkt->guncel_bakiye, 2, ',', '.') }} ₺</td>
                                        @php
                                        // Banka adı ve son güncel bakiyeyi kaydediyoruz
                                        $bankaAdi = $bankaraporitem->bankahrkt->banka->banka_adi;
                                        $bankalarSonGuncelBakiye[$bankaAdi] = $bankaraporitem->bankahrkt->guncel_bakiye;
                                        @endphp
                                    </tr>
                        ...
                        <div class="row mt-1" style="float: right;">
                            <div class="col-md-12">
                                <table class="total-table">
                                    @foreach ($bankalarSonGuncelBakiye as $bankaAdi => $guncelBakiye)
                                        <tr>
                                            <td>{{ $bankaAdi }} Güncel Bakiye</td>
                                            <td>{{ number_format($guncelBakiye, 2, ',', '.') }} ₺</td>
                                        </tr>
                                    @endforeach
                                </table>
                            </div>
                        ...
                        {{-- <div class="row mt-1" style="float: right;" >
                            <div class="col-md-12">
                                <table class="total-table">
                                    ...
                                    <tr>
                                        <th>Toplam Tahsilat Tutarı</th>
                                        <td>{{ number_format($tahsilat_toplam, 2, ',', '.') }} ₺</td>
                                    </tr>
                                </table>
                            </div>
                        </div> --}}

                        {{-- <p class="table-title text-center mt-35"><b>En Çok Tahsilat Yapılan 5 Firma:</b></p>
                        <table class="invoice-table table-stripe4 theme-color">
                            <thead>
                                <tr>
                                    <th style="width: 15px;  text-align: center;">#</th>
                                    <th>Firma Adı</th>
                                    <th>Toplam Tahsilat Tutar</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach ($enCokTahsilatFirmalari as $sn => $enCokTahsilatFirmalariitem)
                                    <tr>
                                        <td style=" text-align: center;"><strong>{{ $sn + 1 }}</strong></td>
                                        <td style=" text-align: center;"><a href="{{ url('cariler/' . $enCokTahsilatFirmalariitem->cari_id) }}">{{ $enCokTahsilatFirmalariitem->firma_unvan }}</a></td>
                                        <td style=" text-align: center;">{{ number_format($enCokTahsilatFirmalariitem->toplam_tahsilat, 2, ',', '.') }} ₺</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table> --}}
