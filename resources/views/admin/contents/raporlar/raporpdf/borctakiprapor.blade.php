...
==============================-->
                                ...
                                <div class="invoice-right">
                                    ...
                                    @else
                                        ...
                                        <address>
                                            ...
                                            {{ $borctakiprapor->first()->firmaadi->firma_unvan }}
                                            <br>
                                            {{ $borctakiprapor->first()->firmaadi->yetkili_kisi }}
                                            <br>
                                            {{ $borctakiprapor->first()->firmaadi->yetkili_kisi_tel }}
                                            <br>
                                            {{ $borctakiprapor->first()->firmaadi->eposta }}
                                    @endif
                    ...
                    @foreach ($borctakiprapor as $borctakipraporitem)
                        @php
                            $firmaId = $borctakipraporitem->firmaadi->id;

                            if (!isset($firmaBakiyeleri[$firmaId])) {
                                $firmaBakiyeleri[$firmaId] = [
                                    'firma_unvan' => $borctakipraporitem->firmaadi->firma_unvan,
                                    'musteri_temsilcisi' => $borctakipraporitem->firmaadi->musteri_temsilcisi,
                                    'yetkili_kisi_tel' => $borctakipraporitem->firmaadi->yetkili_kisi_tel,
                                    'toplam_borc' => 0,
                                    'toplam_alacak' => 0,
                            ...
                            $firmaBakiyeleri[$firmaId]['toplam_borc'] += $borctakipraporitem->borc;
                            $firmaBakiyeleri[$firmaId]['toplam_alacak'] += $borctakipraporitem->alacak;

                            // Son ödeme tarihi kontrolü
                            if (!empty($borctakipraporitem->tahsilat_id) &&
                                (is_null($firmaBakiyeleri[$firmaId]['son_odeme_tarihi']) || $borctakipraporitem->created_at > $firmaBakiyeleri[$firmaId]['son_odeme_tarihi'])) {
                                $firmaBakiyeleri[$firmaId]['son_odeme_tarihi'] = $borctakipraporitem->created_at;
                            }

                            // Toplam değerleri hesaplama
                            $toplam_borc += $borctakipraporitem->borc;
                            $toplam_alacak += $borctakipraporitem->alacak;
                        @endphp
                    @endforeach
                    ...
                    <table class="invoice-table table-stripe4 theme-color">
                        <thead>
                            <tr>
                                ...
                                <th>Toplam Alacak</th>
                                <th>Toplam Bakiye</th>
                                <th>Son Ödeme Yapılan Tarih</th>
                            </tr>
                        </thead>
                        <tbody>
                            @php $sn = 1; @endphp
                            @foreach ($firmaBakiyeleri as $firma)
                                @php
                                    $bakiye = $firma['toplam_borc'] - $firma['toplam_alacak'];
                                @endphp
                                <tr>
                                    <td style="text-align: center;"><strong>{{ $sn++ }}</strong></td>
                                    <td style="text-align: center;">{{ $firma['firma_unvan'] }}</td>
                                    <td style="text-align: center;">{{ $firma['musteri_temsilcisi'] }}</td>
                                    <td style="text-align: center;">{{ $firma['yetkili_kisi_tel'] }}</td>
                                    <td style="text-align: center;">
                                        {{ number_format($firma['toplam_borc'], 2, ',', '.') }} ₺
                                    </td>
                                    <td style="text-align: center;">
                                        {{ number_format($firma['toplam_alacak'], 2, ',', '.') }} ₺
                                    </td>
                                    <td style="text-align: center;">
                                        {{ number_format($bakiye, 2, ',', '.') }} ₺
                                    </td>
                                    <td style="text-align: center;">
                                        {{ $firma['son_odeme_tarihi'] ? $firma['son_odeme_tarihi']->format('d-m-Y') : '-' }}
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
                        ...
                        <!-- Toplam Borç, Alacak ve Bakiye -->
                        <div class="row" style="float: right">
                            <div class="col-md-12">
                                <table class="total-table">
                                    <tr>
                                        <th>TOPLAM BORÇ</th>
                                        <td>{{ number_format($toplam_borc, 2, ',', '.') }} ₺</td>
                                    </tr>
                                    <tr>
                                        <th>TOPLAM ALACAK</th>
                                        <td>{{ number_format($toplam_alacak, 2, ',', '.') }} ₺</td>
                                    </tr>
                                    <tr>
                                        <th>BAKİYE</th>
                                        <td>{{ number_format($toplam_bakiye, 2, ',', '.') }} ₺</td>
                                    </tr>
                                </table>
                            </div>
                        </div>
