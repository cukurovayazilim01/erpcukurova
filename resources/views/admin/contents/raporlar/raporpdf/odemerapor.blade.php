...
==============================-->
                        <header class="th-header header-layout4">
                            <div class="row align-items-center justify-content-between">
                                ...
                                <div class="col-auto">
                                    <h1 class="big-title">Ödeme Rapor</h1>
                            ...
                            <div class="col-auto">
                                <div class="invoice-right">
                                    ...
                                    <address>
                                        ...
                                        {{ $odemerapor->first()->firmaadi->firma_unvan }}
                                        <br>
                                        {{ $odemerapor->first()->firmaadi->yetkili_kisi }}
                                        <br>
                                        {{ $odemerapor->first()->firmaadi->yetkili_kisi_tel }}
                                        <br>
                                        {{ $odemerapor->first()->firmaadi->eposta }}
                                        @endif
                        ...
                        <table class="invoice-table table-stripe4 theme-color">
                            ...
                            <tbody>
                                @foreach ($odemerapor as $sn => $odemeraporitem)
                                    <tr>
                                        <td style=" text-align: center;"><strong>{{ $sn + 1 }}</strong></td>
                                        <td style=" text-align: center;">  <a href="{{ url('tahsilat/' . $odemeraporitem->odeme_id) }}">{{ $odemeraporitem->odeme->odeme_kodu_text }}-{{ $odemeraporitem->odeme->odeme_kodu }}</a></td>
                                        <td style=" text-align: center;">{{ $odemeraporitem->islem_tarihi }}</td>
                                        @if (!$cari_id)
                                        <td style=" text-align: center;">
                                            {{ $odemeraporitem->firmaadi->firma_unvan }}
                                        </td>
                                        @else

                                        @endif
                                        <td style=" text-align: center;">{{ $odemeraporitem->odeme->odeme_turu }}</td>
                                        <td style=" text-align: center;">{{ $odemeraporitem->odeme->user->ad_soyad }}</td>
                                        <td style=" text-align: center;">
                                            @if ( $odemeraporitem->odeme->odeme_tipi == 'Banka')
                                                {{ $odemeraporitem->odeme->bankaadi->banka_adi }}
                                            @elseif( $odemeraporitem->odeme->odeme_tipi == 'Kasa')
                                                    {{ $odemeraporitem->odeme->kasaadi->kasa_adi}}
                                            @endif
                                        </td>


                                        <td style=" text-align: center;">{{ number_format($odemeraporitem->odeme->odeme_tutar, 2, ',', '.') }} ₺</td>
                                    </tr>
                                    @php
                                        $odeme_toplam += $odemeraporitem->odeme->odeme_tutar;
                                    @endphp
                                @endforeach
                            </tbody>
                        </table>
                            ...
                            <div class="col-md-12">
                                <table class="total-table">
                                    ...
                                    <tr>
                                        ...
                                        <td>{{ number_format($odeme_toplam, 2, ',', '.') }} ₺</td>
                                    </tr>
                                </table>
                            </div>
                        </div>

                        <p class="table-title text-center mt-35"><b>En Çok Ödeme Yapılan 5 Firma:</b></p>
                        <table class="invoice-table table-stripe4 theme-color">
                            <thead>
                                <tr>
                                    <th style="width: 15px;  text-align: center;">#</th>
                                    <th>Firma Adı</th>
                                    <th>Toplam Ödeme Tutar</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach ($enCokOdemeFirmalari as $sn => $enCokOdemeFirmalariitem)
                                    <tr>
                                        <td style=" text-align: center;"><strong>{{ $sn + 1 }}</strong></td>
                                        <td style=" text-align: center;"><a href="{{ url('cariler/' . $enCokOdemeFirmalariitem->cari_id) }}">{{ $enCokOdemeFirmalariitem->firma_unvan }}</a></td>
                                        <td style=" text-align: center;">{{ number_format($enCokOdemeFirmalariitem->toplam_odeme, 2, ',', '.') }} ₺</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
