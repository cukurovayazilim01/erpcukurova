...
==============================-->
                        <header class="th-header header-layout4">
                            <div class="row align-items-center justify-content-between">
                                ...
                                <div class="col-auto">
                                    <h1 class="big-title">Kdv Rapor</h1>
                        ...
                        <table class="invoice-table table-stripe4 theme-color">
                            ...
                            <tbody>
                                @foreach ($kdvrapor as $sn => $kdvraporitem)
                                    <tr>
                                        <td style="text-align: center;"><strong>{{ $sn + 1 }}</strong></td>

                                        @if ($kdvraporitem->satis_id)
                                            @php
                                                $toplamSatisFatura++;
                                                $toplamSatisKdv += $kdvraporitem->satis->satis_kdv_toplam;
                                            @endphp
                                            <td style="text-align: center;">
                                                <a href="{{ url('satislar/' . $kdvraporitem->satis_id) }}">
                                                    {{ $kdvraporitem->satis->satis_kodu_text }}-{{ $kdvraporitem->satis->satis_kodu }}
                                                </a>
                                            </td>
                                        @elseif ($kdvraporitem->alis_id)
                                            @php
                                                $toplamAlisFatura++;
                                                $toplamAlisKdv += $kdvraporitem->alis->toplam_kdv_tutar;
                                            @endphp
                                            <td style="text-align: center;">
                                                <a href="{{ url('alislar/' . $kdvraporitem->alis_id) }}">
                                                    {{ $kdvraporitem->alis->alis_kodu_text }}-{{ $kdvraporitem->alis->alis_kodu }}
                                                </a>
                                            </td>
                                        @else
                                        ...
                                        <td style="text-align: center;">{{ $kdvraporitem->islem_tarihi }}</td>

                                            <td style="text-align: center;">
                                                {{ $kdvraporitem->firmaadi->firma_unvan }}
                                            </td>
                                            <td style="text-align: center;">
                                                {{ $kdvraporitem->firmaadi->yetkili_kisi_tel }}
                                            </td>
                                            <td style="text-align: center;">
                                                {{ $kdvraporitem->firmaadi->musteri_temsilcisi }}
                                            </td>

                                        <td style="text-align: center;">
                                            {{ number_format($kdvraporitem->satis_id ? $kdvraporitem->satis->satis_ara_toplam : $kdvraporitem->alis->toplam_ara_toplam, 2, ',', '.') }}
                                            ₺
                                        </td>
                                        <td style="text-align: center;">
                                            {{ number_format($kdvraporitem->satis_id ? $kdvraporitem->satis->satis_kdv_toplam : $kdvraporitem->alis->toplam_kdv_tutar, 2, ',', '.') }}
                                            ₺
                                        </td>
                                        <td style="text-align: center;">
                                            {{ number_format($kdvraporitem->satis_id ? $kdvraporitem->satis->satis_kdvli_toplam : $kdvraporitem->alis->toplam_tutar, 2, ',', '.') }}
                                            ₺
                                        </td>
                                    </tr>
                                @endforeach
                        ...
                        <p class="table-title text-center mt-35"><b>Özet Bilgiler:</b></p>
                        <table class="invoice-table table-stripe4 theme-color">
                            <thead>
                                <tr>
                                    <th style="text-align: center;">Toplam Alış Faturası</th>
                                    <th style="text-align: center;">Toplam Satış Faturası</th>
                                    <th style="text-align: center;">Toplam Alış KDV</th>
                                    <th style="text-align: center;">Toplam Satış KDV</th>
                                    <th style="text-align: center;">Net KDV</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr>
                                    <td style="text-align: center;">{{ $toplamAlisFatura }}</td>
                                    <td style="text-align: center;">{{ $toplamSatisFatura }}</td>
                                    <td style="text-align: center;">{{ number_format($toplamAlisKdv, 2, ',', '.') }} ₺
                                    </td>
                                    <td style="text-align: center;">{{ number_format($toplamSatisKdv, 2, ',', '.') }} ₺
                                    </td>

                                    <td style="text-align: center;">
                                        <b>{{ number_format($toplamSatisKdv - $toplamAlisKdv, 2, ',', '.') }} ₺</b>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
