...
==============================-->
                        <header class="th-header header-layout4">
                            <div class="row align-items-center justify-content-between">
                                ...
                                <div class="col-auto">
                                    <h1 class="big-title">Kasa Rapor</h1>
                                ...
                                <div class="invoice-right">
                                    ...
                                    @else
                                        ...
                                        <address>
                                            ...
                                            {{ $kasarapor->first()->kasahrkt->kasa->kasa_adi }}
                                            <br>
                                    @endif
                        ...
                        @php
                            // Kasa bazında son güncel bakiyeyi tutacak dizi
                            $kasalarSonGuncelBakiye = [];
                        @endphp
                        ...
                        <table class="invoice-table table-stripe4 theme-color">
                            ...
                            <tbody>
                                @foreach ($kasarapor as $sn => $kasaraporitem)
                                    <tr>
                                        <td style=" text-align: center;"><strong>{{ $sn + 1 }}</strong></td>
                                        <td style=" text-align: center;">{{ $kasaraporitem->islem_tarihi }}</td>
                                        @if (!$kasalar)
                                            <td style=" text-align: center;">
                                                {{ $kasaraporitem->kasahrkt->kasa->kasa_adi }}
                                            </td>
                                        @endif

                                        <td style=" text-align: center;">
                                            @if (isset($kasaraporitem->kasahrkt->odeme_id))
                                                {{ $kasaraporitem->kasahrkt->odeme->user->ad_soyad }}
                                            @elseif (isset($kasaraporitem->kasahrkt->tahsilat_id))
                                                {{ $kasaraporitem->kasahrkt->tahsilat->odeme_yapan }}
                                            @endif
                                        </td>

                                        <td style=" text-align: center;">
                                            {{ number_format($kasaraporitem->kasahrkt->bakiye, 2, ',', '.') }} ₺</td>

                                        <td style=" text-align: center;">
                                            @if ($kasaraporitem->kasahrkt->hareket_tipi == 'Gelen Nakit')
                                                <span
                                                    style="color: green">{{ number_format($kasaraporitem->kasahrkt->eklenen_tutar, 2, ',', '.') }}
                                                    ₺</span>
                                            @elseif ($kasaraporitem->kasahrkt->hareket_tipi == 'Giden Nakit')
                                                <span
                                                    style="color: red">{{ number_format($kasaraporitem->kasahrkt->eklenen_tutar, 2, ',', '.') }}
                                                    ₺</span>
                                            @endif
                                        </td>

                                        <td style=" text-align: center;">
                                            {{ number_format($kasaraporitem->kasahrkt->guncel_bakiye, 2, ',', '.') }} ₺
                                        </td>

                                        @php
                                            // Kasa adı ve son güncel bakiyeyi kaydediyoruz
                                            $kasaAdi = $kasaraporitem->kasahrkt->kasa->kasa_adi;
                                            $kasalarSonGuncelBakiye[$kasaAdi] = $kasaraporitem->kasahrkt->guncel_bakiye;
                                        @endphp
                                    </tr>
                                @endforeach
                            </tbody>
                        ...
                        <div class="row mt-1" style="float: right;">
                            <div class="col-md-12">
                                <table class="total-table">
                                    @foreach ($kasalarSonGuncelBakiye as $kasaAdi => $guncelBakiye)
                                        <tr>
                                            <td>{{ $kasaAdi }} Güncel Bakiye</td>
